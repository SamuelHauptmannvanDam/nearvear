...
const centerGrowthBox = document.createElement('centerGrowthBox');
centeringBox.appendChild(centerGrowthBox);

const centeringVerticallyBox = document.createElement('centeringVerticallyBox');
centerGrowthBox.appendChild(centeringVerticallyBox);
...
// // description.innerText = "We all need someone to confess to"
...
textarea.placeholder = "What's your confession - We all need someone to talk to";
...
function sendQuestion() {
    ...
    if(document.getElementById("myTextArea").value == ""){
      document.getElementById("error").innerText = "You've forgot to tell how you feel"
      document.getElementById("error").style.color = "black";
    } else if(document.getElementById("myTextArea").value.length < 140){
      let count = 140 - document.getElementById("myTextArea").value.length;
      document.getElementById("error").innerText = "Short messages create short conversations. How you feel shouldn't be brushed aside. At least 140 characters: " + count + " charachters to go"
      document.getElementById("error").style.color = "black";
    } else if((Object.values(firebaseTimestamp)-Object.values(lastConfession))<3600000){
      document.getElementById("error").innerText = "Confessions should take some effort. You can only confess once every 60 minutes. Help others while you wait."
      document.getElementById("error").style.color = "black";
    } else {


        guessLanguage.name(document.getElementById("myTextArea").value, function(languageName) {
          ...
          firebase.auth().onAuthStateChanged(function(user) {
            if (user) {
              ...
              // SET WHEN YOU LAST CONFESSED
              database.ref('lastConfession').child(uid).set({
                  
                  lastConfessions: firebase.database.ServerValue.TIMESTAMP
                  
              });  
              
              // ----------------------- ADD CONFESSION TO ALL QUESTIONS BY LANGUAGE AND ALL QUESTIONS!
              // ----------------------- FOR ALL QUESTIONS BY LANGUAGE
              database.ref('allQuestionsByLanguage').child(languageName).child(pushID).set({
              ...
              // ----------------------- FOR ALL QUESTIONS
              database.ref('allQuestions').child(pushID).set({
...
function checkDescription(){
  firebase.auth().onAuthStateChanged(function(user) {
      if (user) {
          ...
          database.ref().child("Settings").child("talk").child("description").child(uid).child("description").once('value').then(function(snapshot) {
              if (snapshot.exists()) {
                  if(snapshot.val() == "off"){
                      // do nothing 
                  }
