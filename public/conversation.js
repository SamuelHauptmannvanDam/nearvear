...
// conversationBox.innerText = "conversationBox"
...
// ------------------------------------------ GETS THE SEND BUTTON OR SEND AND ANCHOR
...
firebase.auth().onAuthStateChanged(function(user) {
    ...
    if (user) {
        ...
        let uid = user.uid;
        let rootRef = database.ref().child("Conversations").child(creatorID).child(answeringID);

        rootRef.on("child_added", snap => {

            let message = snap.child("message").val(); //The message
            let creatorID = snap.child("creatorID").val(); // poster of message ID, to give color to your posts.
